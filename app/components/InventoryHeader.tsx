"use client";
export default function InventoryHeader({ statusLocation, setStatusLocation, archiveTab, setArchiveTab, urgentTab, setUrgentTab, setIsMenuOpen, isMenuOpen, isUnlocked, signOut, checkClosing, setShowAuthModal }: any) {
  return (
    <>
            ...
            <div className="absolute top-4 right-4 w-56 bg-[#FDFBF7]/90 backdrop-blur-xl rounded-3xl border border-white/50 shadow-2xl overflow-hidden pointer-events-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-2 flex flex-col gap-1">
                {[
                  ...
                  <button key={item.id} onClick={() => { if (item.action) item.action(); else setStatusLocation(item.id); setIsMenuOpen(false); }} className="w-full text-left px-4 py-3 rounded-2xl hover:bg-black/5 font-bold text-[14px] text-[#3E2723]">{item.label}</button>
                ))}
