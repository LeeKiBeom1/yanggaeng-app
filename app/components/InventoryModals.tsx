          ...
          <div className="bg-[#FDFBF7] w-full max-w-sm rounded-[32px] p-8 border border-[#EFE9E1] shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-4 gap-2 mb-6 max-h-40 overflow-y-auto p-2 border border-[#F5F0E9] rounded-2xl bg-white text-center shadow-inner">
              {YANGGANG_TYPES.map(p => (
                <button key={p} onClick={() => setSelectedProduct(`${p} 양갱`)} className={`py-2 text-[10px] rounded-xl border font-bold transition-all ${selectedProduct === `${p} 양갱` ? "bg-[#5D2E2E] text-white border-[#5D2E2E] shadow-md" : "bg-white text-[#A68966] border-[#F5F0E9] hover:bg-[#F9F5F0]"}`}>{p}</button>
              ))}
            </div>
            <input type="date" value={expiryDate} onChange={(e) => setExpiryDate(e.target.value)} className="w-full mb-4 p-4 border border-[#F5F0E9] rounded-2xl font-bold text-center bg-white text-[#3E2723] focus:border-[#5D2E2E] outline-none shadow-sm transition-all" />
      ...
      {showAuthModal && (
        <div className="fixed inset-0 bg-black/60 z-[760] flex items-center justify-center p-6 backdrop-blur-sm">
          <div className="bg-[#FDFBF7] w-full max-w-sm rounded-[32px] p-8 border border-[#EFE9E1] shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[11px] text-center text-[#A68966] font-bold mb-8 tracking-tight">아이디와 비밀번호를 입력해주세요.</p>
            ...
            <button onClick={signIn} disabled={isAuthLoading} className="w-full py-4 bg-[#5D2E2E] text-white rounded-2xl font-bold shadow-lg hover:bg-[#3E2723] disabled:bg-[#F5F0E9] disabled:text-[#A68966] transition-all">
