...
export default function InventoryModals(props: any) {
  ...
  const handleClosingNext = async (val: number) => {
    ...
    else { saveClosing({}); setStatusLocation("TOTAL"); }
  };
    ...
    <>
            ...
            {statusLocation === "URGENT" && urgentTab !== "STORAGE" ? (
              ...
              <>
                <div className="grid grid-cols-4 gap-2 mb-6 max-h-40 overflow-y-auto p-2 border border-[#F5F0E9] rounded-2xl bg-white text-center shadow-inner">
                  {(statusLocation === "SET" ? SET_TYPES : YANGGANG_TYPES).map((p: any) => {
                ...
                <input type="date" value={expiryDate} onChange={(e) => setExpiryDate(e.target.value)} className="w-full mb-4 p-4 border border-[#F5F0E9] rounded-2xl font-bold text-center bg-white outline-none text-[16px]" />
                {statusLocation === "SET" && (
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button onClick={saveInventory} disabled={isSaving} className={`w-full py-4 text-white rounded-2xl font-bold shadow-lg ${isSaving ? "bg-[#D1C4B5]" : "bg-[#5D2E2E]"}`}>{isSaving ? "저장 중..." : "확인"}</button>
                </div>
              </>
            )}
          </div>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 bg-black/60 z-[760] flex items-center justify-center p-6 backdrop-blur-sm">
          <div className="bg-[#FDFBF7] w-full max-w-sm rounded-[32px] p-8 border border-[#EFE9E1] shadow-2xl">
            ...
            <input type="text" value={loginId} onChange={(e) => setLoginId(e.target.value)} placeholder="아이디" className="w-full mb-3 p-4 border border-[#F5F0E9] rounded-2xl font-bold text-sm bg-white outline-none text-[#3E2723]" />
            <input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} placeholder="비밀번호" className="w-full mb-8 p-4 border border-[#F5F0E9] rounded-2xl font-bold text-sm bg-white outline-none text-[#3E2723]" />
      ...
      {closingDetail && (
        <div className="fixed inset-0 bg-black/60 z-[850] flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setClosingDetail(null)}>
          <div className="bg-[#FDFBF7] w-full max-w-lg rounded-[32px] p-6 border shadow-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4"><h3 className="font-bold text-[#5D2E2E]">{closingDetail.closing_date} 상세 재고</h3><button onClick={() => setClosingDetail(null)} className="text-[#A68966] text-xl">×</button></div>
            <div className="overflow-y-auto flex-1 rounded-xl border border-[#EFE9E1]">
              <table className="w-full text-center table-fixed border-collapse bg-white">
                <thead className="sticky top-0 bg-[#F9F5F0] text-[#5D2E2E] text-[10px] font-bold border-b border-[#EFE9E1]">
                  <tr><th className="py-2 w-[40%] border-r border-[#EFE9E1]">품목</th><th className="py-2 w-[20%] border-r border-[#EFE9E1]">홀</th><th className="py-2 w-[20%] border-r border-[#EFE9E1]">창고</th><th className="py-2 w-[20%]">합계</th></tr>
                </thead>
                <tbody className="divide-y divide-[#F5F0E9] text-[12px]">
                  {closingDetail.stock_snapshot?.map((s: any) => (
                    <tr key={s.product_name}><td className="py-2 border-r border-[#F5F0E9] font-medium text-left px-3">{s.product_name.replace(" 양갱", "")}</td><td className="py-2 border-r border-[#F5F0E9]">{s.floor}</td><td className="py-2 border-r border-[#F5F0E9]">{s.warehouse}</td><td className="py-2 font-bold text-[#5D2E2E] bg-[#FDFBF7]">{s.floor + s.warehouse}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
      ...
      {editTarget && (
        <div className="fixed inset-0 bg-black/60 z-[700] flex items-center justify-center p-6" onClick={() => setEditTarget(null)}>
          <div className="bg-[#FDFBF7] w-full max-w-sm rounded-[32px] p-8 border shadow-2xl text-center" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={confirmEdit} className="w-full py-4 bg-[#5D2E2E] text-white rounded-2xl font-bold shadow-lg">확인</button>
          </div>
        </div>
      )}
