...
export default function InventoryList({
  statusLocation, items, setInventoryItems, urgentItems, usageLogs = [], disposalLogs = [], urgentTab = "STORAGE",
  YANGGANG_TYPES, SET_TYPES, ensureAuthenticated, setEditTarget, setMoveTarget, setMoveQty, setDeleteMode, setDeleteTarget, setShowMoveUrgentModal, setMoveUrgentTarget,
}: InventoryListProps) {
        ...
        {logs.map((log) => (
          <div key={log.id} className="p-4 flex justify-between items-center bg-[#FDFBF7]/50">
            <div>
              ...
              <div className="text-[10px] text-[#A68966] mt-0.5">{new Date(log.created_at).toLocaleDateString()} 처리</div>
            </div>
            <div className="flex items-center gap-3">
              {/* [요구사항 1 반영] 유통기한-갯수 레이아웃 */}
              <span className="text-[12px] font-bold text-[#A68966]">{fmtDate(log.expiry_date)} - </span>
              <span className="font-black text-[#3E2723] text-sm">{log.quantity}개</span>
              <button onClick={() => { if(ensureAuthenticated()){ setDeleteMode(urgentTab === "USAGE" ? "usage" : "disposal"); setDeleteTarget(log); } }} className="text-[#DC3545] font-bold px-2 text-lg active:scale-75 transition-all">×</button>
            </div>
          </div>
        ))}
        {logs.length === 0 && <div className="py-24 text-center text-sm text-[#A68966] italic">기록이 없습니다.</div>}
      </div>
    );
  }
  ...
  return (
    <div className="w-full space-y-4 mb-12">
      {currentTypes.map((name: string) => {
        ...
        let list: any[] = [];
        if (statusLocation === "SET") list = setInventoryItems.filter((i) => i.set_name === pName);
        else if (statusLocation === "URGENT") list = urgentItems.filter((i) => i.product_name === pName);
        else list = items.filter((i) => i.product_name === pName && i.location === statusLocation);
