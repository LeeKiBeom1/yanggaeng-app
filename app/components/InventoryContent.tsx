...
export default function InventoryContent(props: ContentProps) {
  const {
    ...
    setShowMoveUrgentModal, setMoveUrgentTarget
  } = props;
      ...
      <div className="space-y-3 mb-12">
        <div className="flex justify-between items-center px-2 mb-2">
          ...
          <button
            ...
            className="text-[10px] font-bold text-[#A68966] hover:text-[#5D2E2E]"
          >
            기록 비우기
          </button>
        </div>
        <div className="bg-white/60 backdrop-blur-sm rounded-3xl border border-[#EFE9E1] shadow-sm divide-y divide-[#F5F0E9]">
          {historyEvents.map((ev: any) => (
            <div key={ev.id} className="px-5 py-4 flex items-center gap-4">
              <div className={`w-10 h-10 shrink-0 rounded-2xl flex items-center justify-center font-bold text-[11px] border ${ev.kind === "MOVE" ? "bg-gray-50 text-gray-400" : ev.delta > 0 ? "bg-[#F0F7F4] text-[#198754]" : "bg-[#FFF5F5] text-[#DC3545]"}`}>
                {ev.kind === "MOVE" ? "이동" : ev.delta > 0 ? "입고" : "출고"}
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-[14px] text-[#3E2723] truncate">{ev.product_name}</div>
                <div className="text-[11px] text-[#A68966] mt-0.5">{fmtDate(ev.expiry_date)} · {ev.kind === "MOVE" ? `${fmtLoc(ev.from)} → ${fmtLoc(ev.to)}` : fmtLoc(ev.location)}</div>
              </div>
              <div className={`font-bold text-sm ${ev.kind === "MOVE" ? "text-[#3E2723]" : ev.delta > 0 ? "text-[#198754]" : "text-[#DC3545]"}`}>
                {ev.kind === "MOVE" ? `${ev.qty}개` : `${ev.delta > 0 ? "+" : ""}${ev.delta}개`}
              </div>
            </div>
          ))}
        ...
        if (statusLocation === "TOTAL") {
          ...
          return (
            <div key={name} className={`bg-white rounded-[24px] border shadow-sm overflow-hidden transition-all ${total === 0 ? "opacity-40 border-[#F5F0E9]" : "border-[#EFE9E1]"}`}>
              <div className="bg-[#F9F5F0] px-4 py-2 border-b border-[#F5F0E9] flex justify-between items-center text-[#5D2E2E]">
                <span className="font-bold text-[13px]">{name}</span>
                <span className="text-[10px] font-medium opacity-60">재고 합계</span>
              </div>
              <div className="p-4 flex items-center justify-between">
                <div className="flex gap-6 text-[12px] font-medium text-[#A68966]">
                  <span>홀 <span className="text-[#3E2723] ml-1">{floorQty}</span></span>
                  <span>창고 <span className="text-[#3E2723] ml-1">{warehouseQty}</span></span>
                </div>
                <span className={`text-[20px] font-black ${total === 0 ? "text-[#D1C4B5]" : "text-[#5D2E2E]"}`}>{total}</span>
              </div>
            </div>
          );
        }
                  ...
                  {filteredItems.sort((a, b) => a.expiry_date.localeCompare(b.expiry_date)).map((item: any) => {
                    ...
                    return (
                      <div key={item.id} className="bg-[#FDFBF7] rounded-xl border border-[#F5F0E9] p-2 flex items-center justify-between gap-1">
                        {/* [불빛 - 날짜 - 수량] 가로 배치 (폰트 크기 통일) */}
                        <div 
                          onClick={() => { if (statusLocation !== "URGENT" && ensureAuthenticated()) { setEditTarget(item); setEditQty(item.quantity); } }}
                        ...
                        <div className="flex items-center gap-1 shrink-0 ml-1">
                          {statusLocation === "FLOOR" && daysLeft <= 14 && (
                            <button 
                              ...
                              className="w-6 h-6 bg-white border border-[#F5F0E9] rounded-md flex items-center justify-center text-orange-500 text-[10px] font-bold shadow-sm active:scale-90"
                            >
                              !
                            </button>
                          )}
                          {statusLocation === "WAREHOUSE" && (
                            <button 
                              ...
                              className="w-6 h-6 bg-white border border-[#F5F0E9] rounded-md flex items-center justify-center text-[11px] shadow-sm"
                            >
                              🚚
                            </button>
                          )}
                          <button 
                            onClick={() => { if (ensureAuthenticated()) { setDeleteMode(statusLocation === "URGENT" ? "urgent" : "inventory"); setDeleteTarget(item); } }} 
                            className="w-6 h-6 bg-[#FFF5F5] border border-[#FFE3E3] text-[#DC3545] rounded-md flex items-center justify-center text-[14px] font-bold shadow-sm active:scale-90"
                          >
                            ×
                          </button>
                        </div>
