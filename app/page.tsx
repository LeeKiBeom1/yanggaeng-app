...
export default function 재고관리페이지() {
  ...
  const [현재위치, set현재위치] = useState<"FLOOR" | "WAREHOUSE" | "URGENT" | "TOTAL" | "HISTORY">("FLOOR"); 
  ...
  const 히스토리_키 = "yanggaeng_history_v1";
  const 임박재고_키 = "yanggaeng_urgent_v1";
  ...
  async function 재고가져오기() {
    ...
    if (error) { 토스트알림("재고 조회 오류"); return; }
    if (data) setItems(data as 재고항목[]);
  }
  ...
  useEffect(() => {
    ...
    try {
      const raw히스토리 = localStorage.getItem(히스토리_키);
      if (raw히스토리) set히스토리목록(JSON.parse(raw히스토리));
      const raw임박 = localStorage.getItem(임박재고_키);
      if (raw임박) set임박재고목록(JSON.parse(raw임박));
    } catch { }
  }, []);

  function 히스토리저장(next: any[]) { set히스토리목록(next); localStorage.setItem(히스토리_키, JSON.stringify(next)); }
  function 히스토리추가(event: any) { 히스토리저장([event, ...히스토리목록].slice(0, 500)); }
  function 임박재고저장(next: 임박항목[]) { set임박재고목록(next); localStorage.setItem(임박재고_키, JSON.stringify(next)); }
  ...
  async function 재고저장() {
    ...
    let 에러 = null;
    if (기존재고) { const { error } = await supabase.from("inventory").update({ quantity: 기존재고.quantity + 입력수량 }).eq("id", 기존재고.id); 에러 = error; }
    else { const { error } = await supabase.from("inventory").insert([{ product_name: 선택품목, quantity: 입력수량, location: 대상위치, expiry_date: 유통기한 }]); 에러 = error; }
    if (에러) { 토스트알림("오류 발생"); return; }
    히스토리추가({ id: crypto.randomUUID(), ts: Date.now(), kind: "IN", product_name: 선택품목, expiry_date: 유통기한, location: 대상위치, delta: 입력수량 });
    토스트알림("✅ 저장 완료"); set입력수량(0); set입력창보이기(false); 재고가져오기();
  }

  async function 수정확정() {
    if (!인증확인() || !수정대상) return;
    const 이전 = Number(수정대상.quantity); const 이후 = Number(수정수량); const 변동 = 이후 - 이전;
    if (이후 <= 0) { await supabase.from("inventory").delete().eq("id", 수정대상.id); 히스토리추가({ id: crypto.randomUUID(), ts: Date.now(), kind: "OUT", product_name: 수정대상.product_name, expiry_date: 수정대상.expiry_date, location: 수정대상.location, delta: -이전 }); }
    else { await supabase.from("inventory").update({ quantity: 이후 }).eq("id", 수정대상.id); 히스토리추가({ id: crypto.randomUUID(), ts: Date.now(), kind: "EDIT", product_name: 수정대상.product_name, expiry_date: 수정대상.expiry_date, location: 수정대상.location, before: 이전, after: 이후, delta: 변동 }); }
  ...
  async function 재고이동() {
    ...
    히스토리추가({ id: crypto.randomUUID(), ts: Date.now(), kind: "MOVE", product_name: 이동대상.product_name, expiry_date: 이동대상.expiry_date, from: 이동대상.location, to: 새위치, qty: 이동수량 });
  ...
  async function 임박재고이동확정() {
    if (!임박이동대상) return;
    const 항목 = 임박이동대상;
    const 다음임박목록: 임박항목[] = [{ id: crypto.randomUUID(), product_name: 항목.product_name, quantity: 항목.quantity, expiry_date: 항목.expiry_date }, ...임박재고목록].slice(0, 1000);
    임박재고저장(다음임박목록);
    await supabase.from("inventory").delete().eq("id", 항목.id);
    히스토리추가({ id: crypto.randomUUID(), ts: Date.now(), kind: "MOVE", product_name: 항목.product_name, expiry_date: 항목.expiry_date, from: "FLOOR", to: "WAREHOUSE", qty: 항목.quantity });
    set임박이동창보이기(false); set임박이동대상(null); 토스트알림("⏰ 임박 재고 전송 완료"); 재고가져오기();
  }

  async function 삭제실행() {
    if (!인증확인() || !삭제대상) return;
    if (삭제모드 === "urgent") { 임박재고저장(임박재고목록.filter((i) => i.id !== 삭제대상.id)); }
    else { 
      const 대상 = 삭제대상 as 재고항목;
      await supabase.from("inventory").delete().eq("id", 대상.id); 
      히스토리추가({ id: crypto.randomUUID(), ts: Date.now(), kind: "OUT", product_name: 대상.product_name, expiry_date: 대상.expiry_date, location: 대상.location, delta: -대상.quantity }); 
    }
